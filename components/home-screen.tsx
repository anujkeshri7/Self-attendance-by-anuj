...
export function HomeScreen({
  ...
  // Filter templates and subjects based on search query
    ...
    <div className="p-4 space-y-6 max-w-6xl mx-auto">
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={onAddTemplate} variant="outline" size="sm" className="flex items-center space-x-2">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
          {filteredTemplates.map((template) => {
            ...
            return (
              <Card
                key={template.id}
                className="cursor-pointer transition-all hover:shadow-lg hover:scale-105 border-gray-200"
                ...
                <CardContent className="p-6">
                  ...
                  <div className="space-y-3">
                    <div className="flex justify-between text-sm">
                      ...
                      <span className="font-semibold text-gray-800">{stats.subjectCount}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                      <span className="text-gray-600">Attendance:</span>
                      <span className="font-semibold text-green-600">{stats.attendancePercentage}%</span>
                    </div>
          ...
          {/* Add Template Card */}
          <Card
            className="cursor-pointer transition-all hover:shadow-lg hover:scale-105 border-dashed border-2 border-gray-300 hover:border-blue-400"
            onClick={onAddTemplate}
          >
            <CardContent className="p-6 flex flex-col items-center justify-center text-center space-y-4 min-h-[200px]">
              <div className="p-4 bg-gray-100 rounded-full hover:bg-blue-50 transition-colors">
                <Plus className="w-8 h-8 text-gray-600 hover:text-blue-600 transition-colors" />
              </div>
              <div>
                <h3 className="font-semibold text-gray-800 text-lg">Add New Template</h3>
                <p className="text-sm text-gray-600">Create a new attendance template</p>
              </div>
            </CardContent>
          </Card>
        </div>
      ...
      {recentSubjects.length > 0 && (
        <div className="space-y-4">
          <h2 className="text-2xl font-bold text-gray-800">Recent Subjects</h2>
